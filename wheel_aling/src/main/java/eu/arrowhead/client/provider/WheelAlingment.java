 ...
 */
public class WheelAlingment extends ArrowheadClientMain {
  ...
  public static void main(String[] args) {
    new WheelAlingment(args);
  }

  private WheelAlingment(String[] args) {
    Set<Class<?>> classes = new HashSet<>(Arrays.asList(WheelAlingResource.class));
    ...
    System.out.println("Waiting for workflow executor to send commands");
    
    while(true) {
	    // Add option to take response file;
	    listenForResult();
	    
	    String dataManagerUrl = props.getProperty("dataman_url","http://localhost:9456/datamanager/historian");
	    String path_upload = props.getProperty("path_upload", "/media/pi/10D3-4C2C/");
	    String filename_result = props.getProperty("filename_result","result.xml");
	    
	    String result = consumeService_upload(dataManagerUrl, path_upload + filename_result);
  ...
  private void loadAndCompilePayloads(boolean fromFile) {
    if (fromFile) {
      ...
      if (!serviceUri.equals(WheelAlingResource.SERVICE_URI)) {
  ...
  private void listenForResult() {
	  System.out.println("Type \"run\" to save the result from the machine or \"stop\"");
